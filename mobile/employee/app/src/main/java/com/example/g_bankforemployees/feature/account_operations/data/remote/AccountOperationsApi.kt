...
interface AccountOperationsApi {
    ...
    @GET("/core/patterns/api/v2/users/{userId}/bank-accounts/{accountNumber}")
    suspend fun getBankAccount(
        @Path("userId") userId: String,
        @Path("accountNumber") accountNumber: String,
    ): Response<BankAccountFullDto>

    @GET("/core/patterns/api/v2/users/{userId}/credit-accounts/{accountNumber}")
    suspend fun getCreditAccount(
        @Path("userId") userId: String,
        @Path("accountNumber") accountNumber: String,
    ): Response<CreditAccountFullDto>

    @GET("/core/patterns/api/v2/users/{userId}/operations/{accountNumber}")
    ...
    @GET("/core/patterns/api/v2/users/{userId}/operations/missed")
    suspend fun getExpiredOperations(
        @Path("userId") userId: String,
    ): Response<List<OperationDto>>
}
